...
LOGIN_URL = "https://apis.pooq.co.kr/login?apikey=E5F3E0D30947AA5440556471321BB6D9&credential=none&device=pc&drm=wm&partner=pooq&pooqzone=none&region=kor&targetage=all"
...
EPG_URL = "https://apis.wavve.com/live/epgs?enddatetime={0}&genre=all&limit=500&offset=0&startdatetime={1}&apikey=E5F3E0D30947AA5440556471321BB6D9&credential={2}&device=pc&drm=wm&partner=pooq&pooqzone=none&region=kor&targetage=all"
...
def doLogin(userId,password):
    try:
        ...
        response = requests.post(LOGIN_URL,data=payloads)
        jsonResp = json.loads(response.text)
...
def createEPG(credential,outpath):
    try:
        start=datetime.datetime.today().strftime("%Y-%m-%d %H:%M")
        end=(datetime.datetime.today() + datetime.timedelta(days=2)).strftime("%Y-%m-%d %H:%M")
        channelList = getEPG(start,end,credential)
        file = open(outpath,'w', encoding='utf8')
